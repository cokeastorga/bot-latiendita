...
declare module "$app/types" {
	export interface AppTypes {
		RouteId(): "/(protected)" | "/" | "/api" | "/api/chat" | "/api/conversations" | "/api/conversations/status" | "/api/image" | "/api/image/[key]" | "/api/manual-reply" | "/api/webhook" | "/api/whatsapp" | "/api/whatsapp/profile" | "/(protected)/configuracion" | "/(protected)/conversaciones" | "/(protected)/conversaciones/[id]" | "/(protected)/dashboard" | "/login" | "/(protected)/perfil" | "/privacidad" | "/recovery" | "/register" | "/(protected)/sandbox" | "/(protected)/sandbox/chat" | "/terminos" | "/(protected)/usuarios";
		RouteParams(): {
			"/api/image/[key]": { key: string };
