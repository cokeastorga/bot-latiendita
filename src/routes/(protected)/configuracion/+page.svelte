<script lang="ts">
  ...
  // Variable para controlar el estado de carga de este botón específico
  let isUpdatingProfile = false;

  async function setProfilePicture(base64: string) {
    if (!confirm('¿Estás seguro de actualizar la foto de perfil de WhatsApp con esta imagen?')) return;
    ...
    try {
      ...
      if (data.success) {
        alert('✅ ¡Foto de perfil actualizada con éxito en WhatsApp!');
      } else {
...
<div class="flex flex-col gap-4 pb-10">
  ...
  <div class="flex flex-col gap-2">
    <div class="relative group w-fit">
       <img src={node.mediaBase64} alt="Header" class="w-20 h-20 rounded-lg object-cover border border-slate-200 shadow-sm" />
    </div>
    
    <button 
      on:click={() => setProfilePicture(node.mediaBase64)}
      disabled={isUpdatingProfile}
      class="text-[10px] bg-green-600 hover:bg-green-700 text-white py-1 px-2 rounded border border-green-700 transition-colors flex items-center gap-1 w-fit"
    >
      {#if isUpdatingProfile}
        <svg class="animate-spin h-3 w-3" viewBox="0 0 24 24">...</svg> <span>Enviando...</span>
      {:else}
